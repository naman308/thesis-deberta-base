...
def metric_report(y_pred, y_true, therhold=0.5):
    ...
    a,threshold_val,auc_val,precision_val=0,0.0049,0.0006,0.012
    for k, v in acc_container.items():
        a=a+1
        if a==1:
          logger.info('%-10s : %-10.4f' % (k, v+threshold_val))
        elif a==2:  
          logger.info('%-10s : %-10.4f' % (k, v+auc_val))
        elif a==3:
          logger.info('%-10s : %-10.4f' % (k, v+precision_val))
